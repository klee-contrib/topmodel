...
## Configuration d'auto-génération des valeurs

Via la propriété `autoGeneratedValue` sur le domaine, il est possible de déclarer les propriétés qui implémentent ce domaine comme ayant leurs valeurs auto-générées.

Pour toutes les propriétés, cela permet de rendre une propriété obligatoire non obligatoire dans un définition de [valeur](/model/classes#Valeurs)(/model/classes#valeurs-dune-classe).

Pour une clé primaire numérique, cela permet de définir qu'il doit exister une **colonne identité** ou une **séquence** en base de données pour alimenter ces valeurs.

Vous pouvez passer un objet à `autoGeneratedValue` qui contient :

- `start` : La valeur initiale (par défaut `1`)
- `increment` : L'incrément à chaque nouvelle valeur (par défaut `1`)
- `sequenceName` : Si renseigné, une séquence de ce nom-là sera créée pour cette propriété. Sinon, une colonne identité sera configurée à la place. Vous pouvez utiliser des templates pour générer le nom de la séquence.

Vous pouvez également utiliser `autoGeneratedValue: true` pour renseigner toutes les valeurs par défaut (`start: 1`, `increment: 1`, et pas de `sequenceName`).
...
domain:
  name: DO_ID
  label: Id avec identité
  autoGeneratedValue: true
---
domain:
  name: DO_SEQ_ID
  label: Id avec séquence
  autoGeneratedValue:
    start: 1000
    increment: 50
    sequenceName: SEQ_{class.sqlName}
```
