...
De plus, les autres classes du modèle pourront **implémenter cette interface**, en la listant dans `implements`. Cela aura pour effet de **recopier les propriétés** de cette interface dans la classe cible (à l'image d'un [décorateur](/model/decorators)). Cette interface sera aussi implémentée dans le code généré.
...
Si vous voulez utiliser un ID auto-incrémenté pour votre clé primaire simple, il faudra [configurer son domaine](/model/domains#configuration-dauto-génération-des-valeurs) avec `autoGeneratedValue`.
