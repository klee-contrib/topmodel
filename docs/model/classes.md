...
- Soit comme une **classe** (en renseigant `enum: class` sur la classe). Par rapport à une classe classique, ses **[propriétés uniques](/model/properties.md#propriété-avec-des-valeurs-et-une-clé-dunicité) peuvent êtres typées comme des enums** (en particulier la clé primaire). De plus, en indiquant **`readonly: true`** sur la classe (ce qui dans le cas général permet de rendre toutes les propriétés `readonly`), **l'ensemble des valeurs pourra être généré de manière statique** sous forme d'instances de la classe dans le code généré.
