...
public class JavascriptConfig : GeneratorConfigBase
{
    ...
    public string ModelRootFullPath => Path.Combine(OutputDirectory, ModelRootPath!).Replace("\\", "/");

    public string GetClassFileName(Class classe)
    {
        return $"{ModelRootFullPath}/{string.Join('/', classe.Namespace.Module.Split('.').Select(m => m.ToDashCase()))}/{classe.Name.ToDashCase()}.ts";
    }

    public string GetEndpointsFileName(ModelFile file)
    {
        ...
        return Path.Combine(OutputDirectory, ApiClientRootPath!, filePath, $"{fileName}.ts").Replace("\\", "/");
    }

    public List<(string Import, string Path)> GetEndpointImports(ModelFile file)
    {
        return file.Endpoints.SelectMany(e => e.ClassDependencies)
            .Select(dep => (
                Import: dep is { Source: IFieldProperty fp }
                    ...
                    : dep.Classe.Name,
                Path: GetImportPathForClass(dep)!))
            .Concat(file.Endpoints.SelectMany(d => d.DomainDependencies).Select(p => (Import: p.TS!.Type.Split("<").First(), Path: p.TS.Import!)))
            .Where(i => i.Path != null)
            .GroupAndSort();
    }

    public string? GetImportPathForClass(ClassDependency dep)
    {
        string target;
        if (dep.Source is IFieldProperty fp)
        {
            ...
            {
                target = GetReferencesFileName(dep.Classe.Namespace.Module);
            }
            else
            {
                return null;
        ...
        {
            target = GetClassFileName(dep.Classe);
        }

        var source = dep.Source switch
        {
            IProperty { Class: Class classe } => GetClassFileName(classe),
            IProperty { Endpoint: Endpoint endpoint } => GetEndpointsFileName(endpoint.ModelFile),
            Class classe => GetClassFileName(classe),
            _ => null
        };
    ...
    public string GetReferencesFileName(string module)
    {
        return $"{ModelRootFullPath}/{string.Join('/', module.Split('.').Select(m => m.ToDashCase()))}/references.ts";
    }
}
