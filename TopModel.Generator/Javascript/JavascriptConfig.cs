...
public class JavascriptConfig : GeneratorConfigBase
{
    ...
    public string? GetImportPathForClass(ClassDependency dep, string tag, IEnumerable<Class> availableClasses)
    {
        ...
        {
            if (fp.GetPropertyTypeName(availableClasses) != fp.Domain.TS!.Type && dep.Classe.Reference && (dep.Classe.Reference || dep.Classe.ReferenceValues.Any()) && dep.Classe.PrimaryKey.SingleOrDefault()?.Domain.AutoGeneratedValue != true)
            {
