...
public class TypescriptDefinitionGenerator : GeneratorBase
{
    ...
    public override IEnumerable<string> GeneratedFiles =>
        Files.SelectMany(f => GetClasses(f.Value)).Select(c => _config.GetClassFileName(c))
            .Concat(Files.SelectMany(f => f.Value.Classes).Select(c => _config.GetReferencesFileName(c.ModelFile.Module)));
    ...
    private void GenerateClasses(ModelFile file)
    {
        ...
        {
            var fileName = _config.GetClassFileName(classe);
    ...
    private void GenerateReferences(string module)
    {
        ...
        if (_config.ModelRootPath != null && classes.Any())
        {
            var fileName = _config.GetReferencesFileName(module);
            var fileInfo = new FileInfo(fileName);
            var directoryInfo = fileInfo.Directory!;
            if (!directoryInfo.Exists)
            {
    ...
    private void GenerateClassFile(string fileName, Class classe)
    {
        ...
        if (domains.Any())
        {
            ...
            fw.WriteLine($"import {{{string.Join(", ", domains.Select(d => d.Name))}}} from \"{arbo}{_config.DomainImportPath}\";");
        }
