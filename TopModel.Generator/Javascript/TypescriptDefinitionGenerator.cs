...
public class TypescriptDefinitionGenerator : GeneratorBase
{
    ...
    public override IEnumerable<string> GeneratedFiles =>
        Files.SelectMany(f => GetClasses(f.Value)).SelectMany(c => _config.Tags.Intersect(c.ModelFile.Tags).Select(tag => _config.GetClassFileName(c, tag)))
            .Concat(Files.SelectMany(f => f.Value.Classes.Where(c => (c.Reference || c.ReferenceValues.Any()) && c.PrimaryKey?.Domain.AutoGeneratedValue != true)).SelectMany(c => _config.Tags.Intersect(c.ModelFile.Tags).Select(tag => _config.GetReferencesFileName(c.ModelFile.Module, tag))))
            .Distinct();
