...
public class TypescriptDefinitionGenerator : GeneratorBase
{
    ...
    public override IEnumerable<string> GeneratedFiles => Files.SelectMany(f => GetClasses(f.Value))
        ...
        .Concat(Files.SelectMany(f => f.Value.Classes)
            .Select(c => c.ModelFile.Module != null
                ? $"{RootPath}/{string.Join('/', c.ModelFile.Module.Split('.').Select(m => m.ToDashCase()))}/references.ts"
                : $"{RootPath}/references.ts"));

    private string RootPath => Path.Combine(_config.OutputDirectory, _config.ModelRootPath!).Replace("\\", "/");
    ...
    private void GenerateReferences(string module)
    {
        ...
        if (_config.ModelRootPath != null && classes.Any())
        {
            var fileName = module != null
                ? $"{RootPath}/{string.Join('/', module.Split('.').Select(m => m.ToDashCase()))}/references.ts"
                : $"{RootPath}/references.ts";
