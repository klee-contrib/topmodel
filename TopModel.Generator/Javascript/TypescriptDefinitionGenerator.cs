...
public class TypescriptDefinitionGenerator : GeneratorBase
{
    ...
    public override IEnumerable<string> GeneratedFiles =>
        Files.SelectMany(f => GetClasses(f.Value)).SelectMany(c => _config.Tags.Intersect(c.ModelFile.Tags).Select(tag => _config.GetClassFileName(c, tag)))
            .Concat(Files.SelectMany(f => f.Value.Classes.Where(c => (c.Reference || c.ReferenceValues.Any()) && c.PrimaryKey.SingleOrDefault()?.Domain.AutoGeneratedValue != true)).SelectMany(c => _config.Tags.Intersect(c.ModelFile.Tags).Select(tag => _config.GetReferencesFileName(c.ModelFile.Module, tag))))
            .Distinct();
    ...
    {
        return file.Classes.Where(classe => !(classe.Reference || classe.ReferenceValues.Any()) || classe.PrimaryKey.SingleOrDefault()?.Domain.AutoGeneratedValue == true).ToList();
    }
            ...
            .GroupBy(t => t.fileName))
        {
            var classes = Classes
                .Where(c => c.ModelFile.Tags.Intersect(group.Select(t => t.tag)).Any()
                    ...
                    && c.PrimaryKey.SingleOrDefault()?.Domain.AutoGeneratedValue != true);
    ...
    private void GenerateReferenceFile(string fileName, IEnumerable<Class> references, string tag)
    {
        ...
        foreach (var reference in references)
        {
            ...
            var valueProperty = reference.PrimaryKey.SingleOrDefault() ?? reference.Properties.OfType<IFieldProperty>().First();
    ...
    private void WriteReferenceDefinition(FileWriter fw, Class classe)
    {
        ...
        fw.Write(classe.Name);
        fw.Write(", valueKey: \"");
        fw.Write((classe.PrimaryKey.SingleOrDefault() ?? classe.Properties.OfType<IFieldProperty>().First()).Name.ToFirstLower());
        fw.Write("\", labelKey: \"");
