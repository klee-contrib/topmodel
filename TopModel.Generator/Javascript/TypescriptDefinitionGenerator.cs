...
public class TypescriptDefinitionGenerator : GeneratorBase
{
    ...
    public override IEnumerable<string> GeneratedFiles =>
        Files.Values.SelectMany(GetClasses).SelectMany(c => _config.Tags.Intersect(c.ModelFile.Tags).Select(tag => _config.GetClassFileName(c, tag)))
            .Concat(Files.Values.SelectMany(GetReferenceClasses).SelectMany(c => _config.Tags.Intersect(c.ModelFile.Tags).Select(tag => _config.GetReferencesFileName(c.ModelFile.Module, tag))))
            .Distinct();
    ...
    private IEnumerable<Class> GetClasses(ModelFile file)
    {
        return file.Classes.Except(GetReferenceClasses(file));
    }

    private IEnumerable<Class> GetReferenceClasses(ModelFile file)
    {
        return file.Classes.Where(c => c.EnumKey != null || c.Reference && !c.ReferenceKey!.Domain.AutoGeneratedValue);
    }
            ...
            .GroupBy(t => t.fileName))
        {
            var classes = Classes
                .Where(c => c.ModelFile.Tags.Intersect(group.Select(t => t.tag)).Any()
                    ...
                    && (c.EnumKey != null || c.Reference && !c.ReferenceKey!.Domain.AutoGeneratedValue));
