...
public class JpaModelGenerator : GeneratorBase
{
    ...
    {
        fw.WriteLine();
        var codeProperty = classe.Properties.OfType<IFieldProperty>().Count() == 1 ? classe.Properties.OfType<IFieldProperty>().First()
    : classe.PrimaryKey.SingleOrDefault()?.Domain.AutoGeneratedValue == true
    ? classe.PrimaryKey.SingleOrDefault() ?? classe.GetProperties(_config, AvailableClasses).OfType<IFieldProperty>().First()
    : classe.PrimaryKey.SingleOrDefault() ?? classe.UniqueKeys.First().Single();
                ...
                if (classe.GetProperties(_config, AvailableClasses).Count > 1)
                {
                    ...
                    lineToWrite += string.Join(", ", classe.GetProperties(_config, AvailableClasses)
                        ...
                        .Select(prop =>
                        {
                            ...
                            if (prop is AssociationProperty ap && codeProperty.PrimaryKey && ap.Association.ReferenceValues.Any(r => r.Value.ContainsKey(ap.Association.PrimaryKey.Single()) && r.Value[ap.Association.PrimaryKey.Single()] == value))
                            {
