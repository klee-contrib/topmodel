...
public class CSharpClassGenerator : GeneratorBase
{
    ...
    private static void GenerateConstProperties(CSharpWriter w, Class item)
    {
        if (item.ReferenceValues.Any())
        {
            ...
            {
                var code = !item.PrimaryKey.Any() || !item.PrimaryKey.Single().Domain.AutoGeneratedValue
                    ? refValue.Value[item.PrimaryKey.SingleOrDefault() ?? item.Properties.OfType<IFieldProperty>().First()]
    ...
    private void GenerateClassDeclaration(CSharpWriter w, Class item)
    {
        ...
        {
            var primaryKey = item.PrimaryKey.SingleOrDefault() ?? item.Properties.OfType<IFieldProperty>().First();
