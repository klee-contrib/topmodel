...
public abstract class AbstractSchemaGenerator
{
    ...
    private IEnumerable<AssociationProperty> WriteTableDeclaration(Class classe, SqlFileWriter writerCrebas, SqlFileWriter? writerUk, SqlFileWriter? writerType, IList<Class> availableClasses)
    {
        ...
        if (_config.IdentityMode == IdentityMode.SEQUENCE && classe.PrimaryKey != null && classe.PrimaryKey.Domain.AutoGeneratedValue && !classe.PrimaryKey.Domain.SqlType!.ToLower().Contains("varchar"))
        {
            writerCrebas.WriteLine("/**");
            ...
            {
                writerCrebas.Write($"{$" start {this._config.IdentityStart}"}");
            }
        ...
        foreach (var property in properties)
        {
            ...
            if (property.PrimaryKey && property.Domain.AutoGeneratedValue && persistentType.Contains("int") && _config.IdentityMode == IdentityMode.IDENTITY)
            {
