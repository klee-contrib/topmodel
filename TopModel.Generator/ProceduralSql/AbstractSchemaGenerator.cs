...
public abstract class AbstractSchemaGenerator
{
    ...
    private IEnumerable<AssociationProperty> WriteTableDeclaration(Class classe, SqlFileWriter writerCrebas, SqlFileWriter? writerUk, SqlFileWriter? writerType, IList<Class> availableClasses)
    {
        ...
        if (_config.Identity.Mode == IdentityMode.SEQUENCE && classe.PrimaryKey.Count() == 1 && classe.PrimaryKey.Single().Domain.AutoGeneratedValue && !classe.PrimaryKey.Single().Domain.SqlType!.ToLower().Contains("varchar"))
        {
            writerCrebas.WriteLine("/**");
        ...
        foreach (var property in properties)
        {
            ...
            if (property.PrimaryKey && property.Domain.AutoGeneratedValue && persistentType.Contains("int") && _config.Identity.Mode == IdentityMode.IDENTITY)
            {
