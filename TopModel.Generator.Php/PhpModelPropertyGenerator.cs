...
public class PhpModelPropertyGenerator(PhpConfig config)
{
    ...
    private static void WriteOneToMany(PhpWriter fw, AssociationProperty property)
    {
        ...
        fw.WriteLine(
            1,
            @$"#[OneToMany(mappedBy: '{(property is ReverseAssociationProperty rap ? rap.ReverseProperty.NameByClassCamel : @$"{property.Class.NameCamel}{property.Role ?? string.Empty}")}', targetEntity: {property.Association.Name}::class)]"
        );
    }
    ...
    private void WriteRegularProperty(PhpWriter fw, Class classe, IProperty property, string tag)
    {
        if (classe.IsPersistent)
        {
            if (property.PrimaryKey)
            {
                ...
                if (property.AutoGeneratedValue)
                {
                    fw.AddImport(@$"Doctrine\ORM\Mapping\GeneratedValue");
                    var strategy =
                        config.Identity.Mode == IdentityMode.NONE
                            ? string.Empty
                            : $@"(strategy: ""{config.Identity.Mode}"")";
                    fw.WriteLine(1, @$"#[GeneratedValue{strategy}]");
                    if (config.Identity.Mode == IdentityMode.SEQUENCE)
                    {
                        var seqName = $"SEQ_{classe.SqlName}";
                        var initialValue =
                            config.Identity.Start != null ? $", initialValue: {config.Identity.Start}" : string.Empty;
                        var increment =
                            config.Identity.Increment != null
                                ? $", allocationSize: {config.Identity.Increment}"
                                : string.Empty;
