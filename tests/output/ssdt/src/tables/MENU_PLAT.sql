...
create table [dbo].[MENU_PLAT] (
	...
	constraint [FK_MENU_PLAT_MENU_MEN_ID] foreign key ([MEN_ID]) references [dbo].[MENU] ([MEN_ID]),
	constraint [FK_MENU_PLAT_PLAT_PLA_ID] foreign key ([PLA_ID]) references [dbo].[PLAT] ([PLA_ID]),
...
create nonclustered index [IDX_MENU_PLAT_MEN_ID_FK]
	on [dbo].[MENU_PLAT] ([MEN_ID] ASC)
go

/* Index on foreign key column for MENU_PLAT.PLA_ID */
create nonclustered index [IDX_MENU_PLAT_PLA_ID_FK]
