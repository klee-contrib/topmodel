...
public abstract class MapperGeneratorBase<T>(ILogger<MapperGeneratorBase<T>> logger, IFileWriterProvider writerProvider)
    : GeneratorBase<T>(logger, writerProvider)
    where T : GeneratorConfigBase
{
    public override IEnumerable<string> GeneratedFiles =>
        FromMappers
            .SelectMany(m => Config.Tags.Intersect(GetMapperTags(m)).Select(tag => GetFileName(m, tag)))
            .Concat(
                ToMappers.SelectMany(m => Config.Tags.Intersect(GetMapperTags(m)).Select(tag => GetFileName(m, tag)))
            )
            .Distinct();
    ...
    {
        var fromMappers = FromMappers
            .SelectMany(mapper =>
                Config
                    .Tags.Intersect(GetMapperTags(mapper))
                ...
                f =>
                {
                    ...
                    return (
                        Mappers: f.Select(m => m.Mapper)
                            ...
                            .ThenBy(
                                m => string.Join(',', m.Mapper.Params.Select(p => p.GetName())),
        ...
        Parallel.ForEach(
            ...
            fileName =>
            {
                ...
                var (fileToMappers, toTags) = toMappers.TryGetValue(fileName, out var value2)
                    ? value2
                    : (Mappers: Array.Empty<(Class, ClassMappings)>(), Tags: Array.Empty<string>());
