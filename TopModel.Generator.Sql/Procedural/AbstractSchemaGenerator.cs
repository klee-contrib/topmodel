...
public abstract class AbstractSchemaGenerator
{
    ...
    protected static Dictionary<string, string?> CreatePropertyValueDictionary(Class modelClass, ClassValue initItem)
    {
        ...
        foreach (var property in modelClass.Properties.OfType<IFieldProperty>())
        {
            if (!property.PrimaryKey || !property.Domain.AutoGeneratedValue)
            {
                ...
                {
                    null or "null" => "null",
                    string s when property.Domain.Sql!.Type.Contains("varchar") || property.Domain.Sql.Type.Contains("timestamp") => $"'{ScriptUtils.PrepareDataToSqlDisplay(s)}'",
                    object v => v.ToString()
                };
            }
        }
    ...
    private IEnumerable<AssociationProperty> WriteTableDeclaration(Class classe, SqlFileWriter writerCrebas, SqlFileWriter? writerUk, SqlFileWriter? writerType, IList<Class> availableClasses)
    {
        ...
        if (Config.Identity.Mode == IdentityMode.SEQUENCE && classe.PrimaryKey.Count() == 1 && classe.PrimaryKey.Single().Domain.AutoGeneratedValue && !classe.PrimaryKey.Single().Domain.Sql!.Type.ToLower().Contains("varchar"))
        {
            writerCrebas.WriteLine("/**");
