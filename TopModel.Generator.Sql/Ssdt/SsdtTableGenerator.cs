    ...
    : ClassGeneratorBase<SqlConfig>(logger, writerProvider)
{
    ...
    {
        var persistentType =
            property is not CompositionProperty ? Config.GetType(property, Classes)
        ...
        if (
            ...
            && Config.GetType(property, Classes).Contains("int")
    ...
    private IEnumerable<IProperty> WriteInsideInstructions(IFileWriter writer, Class table)
    {
        ...
        foreach (
            var property in properties
                ...
                .Where(ap => ap.Association.IsPersistent && Classes.Contains(ap.Association))
        )
        {
            sb.Clear();
