    ...
    : ClassGeneratorBase<SqlConfig>(logger, writerProvider)
{
    ...
    {
        var persistentType =
            property is { Composition: null } ? Config.GetType(property, forceAssociationPropertyType: true)
        ...
        if (
            ...
            && Config.GetType(property, forceAssociationPropertyType: true).Contains("int")
    ...
    private void WriteConstraintForeignKey(StringBuilder sb, IProperty property)
    {
        ...
        {
            sb.Append(
                $"constraint FK_{tableName}_{referenceClass.SqlName}_{propertyName} foreign key ({propertyName}) references {referenceClass.SqlName} ({property.AssociationProperty!.SqlName})"
            );
        }
    }
