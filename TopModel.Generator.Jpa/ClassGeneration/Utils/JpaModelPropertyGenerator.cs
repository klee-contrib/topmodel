...
public class JpaModelPropertyGenerator(JpaConfig config, IDictionary<string, string> newableTypes)
{
    ...
    protected virtual IEnumerable<JavaAnnotation> GetAnnotations(IProperty property, string tag)
    {
        ...
        {
            foreach (var a in GetIdAnnotations(property, tag))
            {
                yield return a;
            }
        }
    ...
    protected virtual IEnumerable<JavaAnnotation> GetAutogeneratedAnnotations(IProperty property, string tag)
    {
        var autoGenerated = new JavaAnnotation("GeneratedValue", imports: "jakarta.persistence.GeneratedValue");

        var sequenceName = Config.GetSequenceName(property, tag);
        if (sequenceName != null)
        {
            autoGenerated
                .AddAttribute("strategy", "GenerationType.SEQUENCE", "jakarta.persistence.GenerationType")
                .AddAttribute("generator", $@"""{sequenceName}""");
            var sequenceGenerator = new JavaAnnotation(
                "SequenceGenerator",
                imports: "jakarta.persistence.SequenceGenerator"
            )
                .AddAttribute("sequenceName", $@"""{sequenceName}""")
                .AddAttribute("name", $@"""{sequenceName}""")
                .AddAttribute("initialValue", $"{property.GeneratedValue!.Start}")
                .AddAttribute("allocationSize", $"{property.GeneratedValue!.Increment}");
        ...
        {
            var type = Config.GetType(property).ToLower();
            if (type == "long" || type.StartsWith("int") || type == "short")
            {
    ...
    protected virtual IEnumerable<JavaAnnotation> GetIdAnnotations(IProperty property, string tag)
    {
        ...
        else
        {
            ...
            if (property.GeneratedValue != null && property.Class?.PrimaryKey.Count() == 1)
            {
                foreach (var a in GetAutogeneratedAnnotations(property, tag))
                {
                    yield return a;
                }
            }
