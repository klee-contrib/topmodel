...
- [#586](https://github.com/klee-contrib/topmodel/pull/586) - Configuration `autoGeneratedValue` pour les colonnes identités / séquences

  **breaking change** : La configuration de colonne identité / séquence se fait désormais dans le domaine dans `autoGeneratedValue`, au lieu de le spécifier dans la configuration des générateurs JPA et SQL. Vous pouvez donc spécifier quelque chose comme :
  ...
  sur le domaine de vos PKs (à priori `DO_ID`) si vous utilisez cette configuration.

  Etant désormais gérée dans TopModel de base, cette fonctionnalité est disponible dans le générateur C#, et vous n'avez plus à vous soucier de configurer vos deux configs de générateurs de la même façon. Vous aurez évidemment besoin de générateurs à jour pour utiliser ces évolutions.

  **mini breaking change** : Les associations utilisées comme clé primaire sont désormais automatiquement considérées comme `autoGeneratedValue: false`, ce qui n'était le cas que dans le générateur SQL auparavant. Cela implique que vous pourrez voir ces PKs devenir obligatoires dans certains contextes (en JS par exemple, ou les PKs autogénérées étant historiquement générées comme `required: false` pour ne pas bloquer la validation, ou en C# dans les mappers `to` ou les propriétés obligatoires sont ajoutées comme paramètres).

- [#581](https://github.com/klee-contrib/topmodel/pull/581) - Interfaces, classes abstraites, et stratégies d'héritage

  **breaking change** : Vous devez remplacer `abstract: true` par `type: interface`, pour retrouver la même génération qu'avant.

  Vous pouvez désormais implémenter des classes de type interface dans des classes via `implements`, et définir de vraies classes abstraites avec `type: abstract`.

  Vous pouvez aussi spécifier la stratégie d'héritage pour vos classes persistées via `inheritanceStrategy`, pour pouvoir choisir d'avoir une seule table pour votre hiérarchie ou bien des tables séparées sans jointures pour toutes vos classes non abstraites, au lieu de devoir forcément avoir une table par classe et des jointures.

  Les nouvelles fonctionnalités auront évidemment besoin de générateurs à jour pour être correctement générées (`type: interface` est converti en équivalent `abstract: true` pour les anciens générateurs).
